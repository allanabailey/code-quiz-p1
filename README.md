...
* The user was able to hit enter or submit on the home screen and increment their correct score when no answer was provided and no question was displayed. This was fixed by adding a condition in the checkAnswer() function that marked any empty user input as incorrect.
    * This was initially fixed by adding the conditions to the functions checking for the submit button and enter key: **if(event.key === 'Enter' && event.target.value !== '' && codingLang !== null)**, however this was then causing multiple issues when the user attempted to submit answers with an active quiz.
